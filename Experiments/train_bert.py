...
def run(args):
    ...
    if len(args.glue_dataset_list) > 0:
        ...
        glue = {'cola': cola, 'sst2': sst2, 'mrpc': mrpc, 'qqp': qqp, 'stsb': stsb, 'mnli': mnli, 'qnli': qnli,
                'rte': rte,
                'wnli': wnli}  # , 'ax': ax
    ...
    for (task_name, label_dict), (_, dataset) in zip(labels.items(), glue.items()):
        ...
        if (hasattr(model.config, "label2id") and
            ...
            if list(sorted(label_name_to_id.keys())) == list(sorted(label_dict["label_list"])):
                label_to_id = {i: int(label_name_to_id[label_dict["label_list"][i]]) for i in
                               range(label_dict["num_labels"])}
            else:
                print(
